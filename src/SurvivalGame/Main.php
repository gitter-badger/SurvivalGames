...
class Main extends PluginBase implements Listener{
	...
	public function onEnable()
	{
		...
		$this->all=0;//最大玩家数量
		$this->config->save();
		$this->getServer()->getLogger()->info(TextFormat::GREEN."[SurvivalGames] Plugin loaded!");
	...
	public function onPlayerCommand(PlayerCommandPreprocessEvent $event)
	{
		...
		default:
			$event->setCancelled();
			$event->getPlayer()->sendMessage("[SurvivalGames] You are now in game,cannot use other commands");
			$event->getPlayer()->sendMessage("[SurvivalGames] You can also use /lobby to exit game");
			break;
		}
		unset($event);
	}
	...
	public function onDamage(EntityDamageEvent $event)
	{
		...
		if ($event instanceof EntityDamageByEntityEvent)
		{
			...
			if($player instanceof Player && $killer instanceof Player)
			{
		    	...
		    	{
		    		$event->setCancelled();
		    		$killer->sendMessage("[SurvivalGames] PLEASE DO NOT INTERFERENCE THE GAME");
		    		$killer->kill();
		    	}
		    }
		}
	...
	public function PlayerDeath(PlayerDeathEvent $event){
		if($this->gameStatus==3 || $this->gameStatus==4)
		{
			if(isset($this->players[$event->getEntity()->getName()]))
			{
				...
				{
					$this->sendToAll("[SurvivalGames] Player {$event->getEntity()->getName()} die");
					$this->sendToAll("[SurvivalGames] Players :".count($this->players));
					$this->sendToAll("[SurvivalGames] Last time :".$this->lastTime." sec");
				}
				$this->changeStatusSign();
			}
	...
	public function gameTimber(){
		...
		if($this->gameStatus==3 || $this->gameStatus==4)
		{
			...
			{
				Server::getInstance()->broadcastMessage("[SurvivalGames] The game ends for all players have been dead");
				$this->gameStatus=0;
				$this->lastTime=0;
				$this->clearChest();
				$this->ClearAllInv();
		...
		if($this->gameStatus==3)
		{
			...
			case 10:
				...
				break;
			case 0:
				$this->sendToAll("[SurvivalGames] the death match begins");
				foreach($this->players as $pl)
				{
					$p=$this->getServer()->getPlayer($pl["id"]);
					$p->setLevel($this->level);
	...
	public function changeStatusSign()
	{
		...
		if($sign instanceof Sign)
		{
			...
			{
			case 0:
				$sign->setText("§6Catching Fire","§bTap to join","amount :".count($this->players),"");
				break;
			case 1:
				$sign->setText("§6Catching Fire","§bTap to join","amount :".count($this->players),"time left :".$this->lastTime."sec");
				break;
			case 2:
				$sign->setText("§6Catching Fire","§bstarting right now","amount :".count($this->players),"you are prohibited :".$this->lastTime."sec");
				break;
			case 3:
